{extends file='page.tpl'}

{block name='page_content'}

    <div class="konfigurator">
		<div class="initial-screen"></div>

		<form class="layout " id="switchConfigurator" style="display:none">
			<label for="color-picker" id="color-picker-label">
				Wybierz kolor ściany
				<input type="color" id="color-picker" value="#ff0000" style="opacity: 0; width: 0; height: 0; position: absolute;"/>

			</label>
			<div class="back-button" title="Obróć zestaw"><i class="material-icons">autorenew</i></div>
			<div class="light-button" title="Włącz podświetlenie"><i class="material-icons">power_settings_new</i></div>

			<input type="hidden" name="category" value="mechanical"/>

			<div class="left-side" id="leftSideContent"></div>
			<div class="right-side">
				<div class="d-flex justify-content-evenly steps">
					<span>Krok 1</span>
					<span>Krok 2</span>
					<span>Krok 3</span>
					<span>Krok 4</span>
					<span>Krok 5</span>
				</div>
				<div class="progress-bar">
					<div class="progress-bar-indicator"></div>
				</div>

				<div class="header">
					<h3 id="step-header">
						Nagłówek
					</h3>
					<div class="amount-wrapper " style="display: none;">


						<div>
							<span class="bold price">cena</span>
							<span class="amount bold " id="totalAmount">99,99zł</span>
						</div>
						<span>brutto</span>

					</div>
				</div>
				<div id="rightSideContent" class="rightSideContent"></div>

				<div class="footer"></div>

			</div>
		</form>
	</div>

<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
<script src="https://unpkg.com/jspdf@3.0.1/dist/jspdf.umd.min.js"></script>
<script src="https://unpkg.com/@popperjs/core@2.11.8/dist/umd/popper.min.js"></script>
<script src="https://unpkg.com/tippy.js@6.3.7/dist/tippy-bundle.umd.min.js"></script>
<script>{literal}
	jQuery('.light-button').click(function(event){
		jQuery( ".box-spot.filed" ).each(function( index ) {
			if(jQuery('.light-button').hasClass('active')){
				var url = jQuery(this).attr('data-front');
			}else{
				var url = jQuery(this).attr('data-light');
			}
			jQuery(this).css('background-image',url)
		});
		
		jQuery(this).toggleClass('active');
	});
	jQuery(document).on("click", '.custom-radio-filter', function (event) {
		
		var colorId = jQuery(this).find('input').val();
		jQuery('.color-button').hide();
		jQuery('.color-button-'+colorId).show();
	});
	jQuery('.back-button').click(function(event){
		jQuery('.light-button').removeClass('active');
		jQuery( ".box-spot.filed" ).each(function( index ) {
			if(jQuery('.back-button').hasClass('active')){
				var url = jQuery(this).attr('data-front');
			}else{
				var url = jQuery(this).attr('data-back');
			}
			jQuery(this).css('background-image',url);
		});
		if(jQuery(this).hasClass('active')){
			jQuery('.light-button').show();
			var frameurl = jQuery('.boxes-configuraiton-preview').attr('data-front');
			jQuery('.konfigurator').removeClass('back');
		}else{
			jQuery('.light-button').hide();
			var frameurl = jQuery('.boxes-configuraiton-preview').attr('data-back');
			jQuery('.konfigurator').addClass('back');
		}
		
		
		jQuery('.boxes-configuraiton-preview').css('background-image',frameurl);
		jQuery(this).toggleClass('active');
	})
	
{/literal}
</script>
<script>
window.jsPDF = window.jspdf.jsPDF;
 var ajaxUrl = "{$link->getModuleLink('konfigurator', 'AjaxGenerateHash')|escape:'javascript'}";
 const productsArray = [
	{foreach from=$products item=product}
		{
			id: {$product.id_product},
			reference: '{$product.reference}',
		},
	{/foreach}
 ];
const configuratoeData = {
  category: {
    title: "Wybierz serię produktów",
    type: "categorySelect",
    name: "category",
    options: [
      {
        value: "mechanical",
        title: "Włączniki mechaniczne",
        imgUrl:
          "/modules/konfigurator/views/img/mechaniczne.png",
      },
      {
        value: "touch",
        title: "Włączniki dotykowe",
        imgUrl:
          "/modules/konfigurator/views/img/dotykowe.jpg",
      },
    ],
  },
  mechanical: {
    color: {
      title: "Wybierz kolor",
      name: "color",
      options: [
        {
          value: 19,
          title: "Złoty",
          price: 0,
          imgUrl:
            "/3237-medium_default/wlacznik-mechaniczny-klawiszowy-pojedynczy-zloty.jpg",
        },
        {
          value: 17,
          title: "Czarny",
          price: 0,
          imgUrl:
            "/3230-home_default/wlacznik-mechaniczny-klawiszowy-pojedynczy-czarny.jpg",
        },
		{
          value: 18,
          title: "Szary",
          price: 0,
          imgUrl:
            "/3234-medium_default/wlacznik-mechaniczny-klawiszowy-pojedynczy-szary.jpg",
        },
		{
          value: 16,
          title: "Biały",
          price: 0,
          imgUrl:
            "/3229-large_default/wlacznik-mechaniczny-pojedynczy-bialy.jpg",
        },
      ],
    },
    material: {
      title: "Wybierz materiał",
      name: "material",
      options: [
        {
          value: "glass",
          title: "Szkło",
          images: [
            {
              url: "/2678-medium_default/ramka-szklana-pojedyncza-czarna.jpg",
              color: 17,
            },
            {
              url: "/2680-medium_default/ramka-szklana-pojedyncza-zlotya.jpg",
              color: 19,
            },
			{
              url: "/2679-medium_default/ramka-szklana-pojedyncza-szara.jpg",
              color: 18,
            },
			{
              url: "/2681-medium_default/ramka-szklana-pojedyncza-biala.jpg",
              color: 16,
            },
          ],
        },
        {
          value: "plastic",
          title: "Plastik",
          images: [
            {
              url: "/3911-medium_default/ramka-pojedyncza-zlota-pc-1f-dc-smart-home.jpg",
              color: 19,
            },
            {
              url: "/3917-medium_default/ramka-pojedyncza-czarna-pc-1f-dc-smart-home.jpg",
              color: 17,
            },
			{
              url: "/3914-medium_default/ramka-pojedyncza-szara-pc-1f-dc-smart-home.jpg",
              color: 18,
            },
			{
              url: "/3919-medium_default/ramka-pojedyncza-biala-pc-1f-dc-smart-home.jpg",
              color: 16,
            },
          ],
        },
      ],
    },
    boxesNumber: {
      title: "Ilość puszek",
      name: "boxesNumber",
      options: [
        {
          value: "1",
          products: [
		  {foreach from=$ramkiszklane1 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "glass",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 86,
			  height: 86,
			  readyprod:'',
			  depth: {$product.depth},
			  konfigurator_text: `{$product.konfigurator_text}`,
				konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},            
			},
		{/foreach}
            {foreach from=$ramkiplastik1 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "plastic",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 86,
			  height: 86,
			  readyprod:'',
			  depth: {$product.depth},
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
          ],
        },
        {
          value: "2",
          products: [
            {foreach from=$ramkiszklane2 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "glass",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 157,
			  height: 86,
			  readyprod:'',
			  depth: {$product.depth},
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
            {foreach from=$ramkiplastik2 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "plastic",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 157,
			  height: 86,
			  readyprod:'',
			  depth: {$product.depth},
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
          ],
        },
        {
          value: "3",
          products: [
            {foreach from=$ramkiszklane3 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "glass",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 228,
			  height: 86,
			  readyprod:'',
			  depth: {$product.depth},
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
            {foreach from=$ramkiplastik3 item=product}
            {
              id: {$product.id_product},
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "plastic",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 228,
			  height: 86,
			  depth: {$product.depth},
			  readyprod:'',
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
          ],
        },
		{
          value: "4",
          products: [
            {foreach from=$ramkiszklane4 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "glass",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 299,
			  height: 86,
			  depth: {$product.depth},
			  readyprod:'',
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
            {foreach from=$ramkiplastik4 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "plastic",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 299,
			  height: 86,
			  depth: {$product.depth},
			  readyprod:'',
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
          ],
        },
		{
          value: "5",
          products: [
            {foreach from=$ramkiszklane5 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "glass",
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 370,
			  height: 86,
			  depth: {$product.depth},
			  readyprod:'',
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
            {foreach from=$ramkiplastik5 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              material: "plastic",             
			  price: {$product.price},
			  color: {$product.id_feature_value},
			  width: 370,
			  height: 86,
			  depth: {$product.depth},
			  readyprod:'',
			  konfigurator_text: `{$product.konfigurator_text}`,
			  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
            },
		{/foreach}
          ],
        },
      ],
    },
    boxesTypes: {
      title: "Konfiguracja wkładów",
      name: "boxesTypes",
      options: [
        {
          name: "Wkłady dzwonkowe",
          type: "doorBell",
          data: [
			{foreach from=$wkdzwonek item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
        {
          name: "Wkłady Standardowe Światła",
          type: "light",
          data: [
            {foreach from=$wkstandard item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Impulsowe",
          type: "impuls",
          data: [
            {foreach from=$wkimpuls item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Krzyżowe",
          type: "cross",
          data: [
            {foreach from=$wkcross item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Rolet",
          type: "rol",
          data: [
            {foreach from=$wkrol item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Schodowe",
          type: "schod",
          data: [
            {foreach from=$wkschody item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Ściemniacze",
          type: "sciem",
          data: [
            {foreach from=$wksciem item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka pojedyncze",
          type: "plug1",
          data: [
            {foreach from=$plug1 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka podwójne",
          type: "plug2",
          data: [
            {foreach from=$plug2 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka z ładowarką USB",
          type: "plug3",
          data: [
            {foreach from=$plug3 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka bryzgoszczelne z klapką",
          type: "plug4",
          data: [
            {foreach from=$plug4 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka Wi-Fi",
          type: "plug5",
          data: [
            {foreach from=$plug5 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka ZigBee",
          type: "plug6",
          data: [
            {foreach from=$plug6 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka TV 1/2",
          type: "plug7",
          data: [
            {foreach from=$plug7 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka SAT 1/2",
          type: "plug8",
          data: [
            {foreach from=$plug8 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka LAN CAT6 1/2",
          type: "plug9",
          data: [
            {foreach from=$plug9 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Gniazdka Pozostałe 1/2",
          type: "plug10",
          data: [
            {foreach from=$plug10 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Czujniki Ruchu",
          type: "plug11",
          data: [
            {foreach from=$plug11 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Ładowarki USB 1/2",
          type: "plug12",
          data: [
            {foreach from=$plug12 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Zaślepki 1/2",
          type: "plug13",
          data: [
            {foreach from=$plug13 item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Termostaty",
          type: "termo",
          data: [
            {foreach from=$termo item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
      ],
    },
    summary: {
      title: "Podsumowanie konfiguracji",
    },
  },
  touch: {
    color: {
      title: "Wybierz kolor",
      name: "color",
      options: [
        {
          value: 19,
          title: "Złoty",
          imgUrl:
            "/img/customproduct/konfigurator_image1_279.png",
        },
        {
          value: 17,
          title: "Czarny",
          imgUrl:
            "/img/customproduct/konfigurator_image1_277.png",
        },
		{
          value: 18,
          title: "Szary",
          imgUrl:
            "/img/customproduct/konfigurator_image1_278.png",
        },
		{
          value: 16,
          title: "Biały",
          imgUrl:
            "/img/customproduct/konfigurator_image1_276.png",
        },
      ],
    },
    boxesNumber: {
      title: "Ilość puszek",
      name: "boxesNumber",
      options: [
        {
          value: "1",
          products: [
            {foreach from=$ramkitouch1 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 86,
			  height: 86,
			  readyprod: "{$product.readyprod}",
			  depth: {$product.depth},
			},
		{/foreach}
          ],
        },
        {
          value: "2",
          products: [
            {foreach from=$ramkitouch2 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 157,
			  height: 86,
			  readyprod: "{$product.readyprod}",
			  depth: {$product.depth},
			},
		{/foreach}
          ],
        },
		{
          value: "3",
          products: [
            {foreach from=$ramkitouch3 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 228,
			  height: 86,
			  readyprod: "{$product.readyprod}",
			  depth: {$product.depth},
			},
		{/foreach}
          ],
        },
		{
          value: "4",
          products: [
            {foreach from=$ramkitouch4 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 299,
			  height: 86,
			  readyprod: "{$product.readyprod}",
			  depth: {$product.depth},
			},
		{/foreach}
          ],
        },
		{
          value: "5",
          products: [
            {foreach from=$ramkitouch5 item=product}
            {
              id: {$product.id_product},
              ean: {$product.ean13}, reference: "{$product.reference}",
			  productUrl:"{$link->getProductLink($product.id_product)}",
			  imgUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  imgUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
			  url2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
			  url3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
              color: {$product.id_feature_value},
			  price: {$product.price},
			  width: 370,
			  height: 86,
			  readyprod: "{$product.readyprod}",
			  depth: {$product.depth},
			},
		{/foreach}
          ],
        },
        
      ],
    },
    boxesTypes: {
      title: "Konfiguracja wkładów",
      name: "boxesTypes",
      options: [
        {
          name: "Wkłady włączniki dotykowe światła",
          type: "light",
          data: [
            {foreach from=$touchlight item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
        {
          name: "Wkłady schodowo-krzyżowe",
          type: "stairsCross",
          data: [
            {foreach from=$touchstairs item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady Wi-Fi",
          type: "touchwifi",
          data: [
            {foreach from=$touchwifi item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady ZigBee",
          type: "touchzigbee",
          data: [
            {foreach from=$touchzigbee item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady do ściemniaczy",
          type: "touchsciem",
          data: [
            {foreach from=$touchsciem item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Wkłady do rolet",
          type: "touchrol",
          data: [
            {foreach from=$touchrol item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
        {
          name: "Gniazda elektryczne",
          type: "basicPlug",
          data: [
            {foreach from=$plug item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
		{
          name: "Termostaty",
          type: "basicTermo",
          data: [
            {foreach from=$termo item=product}
			{
				  id: {$product.id_product},
				  ean: {$product.ean13}, reference: "{$product.reference}", productUrl:"{$link->getProductLink($product.id_product)}",
				  name: "{$product.name}",
				  inputUrl:"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}",
				  previewUrl: {if !$product.konfigurator_image1}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image1}"{/if},
				  previewUrl2: {if !$product.konfigurator_image2}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.image2, 'home_default')}"{else}"{$product.konfigurator_image2}"{/if},
				  previewUrl3: {if !$product.konfigurator_image3}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_image3}"{/if},
				  price: {$product.price},
				  color: {$product.id_feature_value},
				  width: {$product.width},
				  height: 86,
				  depth: {$product.depth},
				  konfigurator_text: `{$product.konfigurator_text}`,
				  konfigurator_schema: {if !$product.konfigurator_schema}"{$link->getImageLink({$product.name|replace:' ':'-'|lower}, $product.id_image, 'home_default')}"{else}"{$product.konfigurator_schema}"{/if},
				},
			{/foreach}
          ],
        },
      ],
    },
    summary: {
      title: "Podsumowanie konfiguracji",
    },
  },
};

const stepsMechanical = [
  "color",
  "material",
  "boxesNumber",
  "boxesTypes",
  "summary",
];
const stepsTouch = ["color", "boxesNumber", "boxesTypes", "summary"];

function initialize() {
  let steps = ["category"];
  let currentStep = "category";
  const leftSideContent = jQuery("#leftSideContent");
  const rightSideContent = jQuery("#rightSideContent");
  const switchConfiguratorForm = jQuery("#switchConfigurator");

  function setLeftSideContent(content, dimensions = "") {
	
    leftSideContent.html(content.hide());
    content.fadeIn();
	leftSideContent.append(dimensions);
		
  }
  function setRightSideContent(content) {
    rightSideContent.html(content).hide();
    rightSideContent.fadeIn();
  }
  function setHeader(text) {
    jQuery("#step-header").text(text);
  }
  function createNavButtons() {
    const wrapper = jQuery("<div>");
    const next = jQuery("<button>")
      .html("Następny krok &gt;")
      .addClass("nav-button nav-button--next");
    const prev = jQuery("<button>")
      .html("&lt; Poprzedni krok")
      .addClass("nav-button nav-button--prev");

    next.click((e) => {
      const currentIndex = steps.indexOf(currentStep);

      e.preventDefault();
      setCurrentStep(steps.at(currentIndex + 1));
    });
    prev.click((e) => {
	  jQuery(".nav-button--next").prop("disabled", false);
      const currentIndex = steps.indexOf(currentStep);

      e.preventDefault();
      setCurrentStep(steps.at(currentIndex - 1));
    });
	const newConfig = jQuery('<a>')
	.html("Nowa konfiguracja")
	.addClass("new-config")
	.attr("id", "ignorePDF")
	.attr("href", "/konfigurator");
    wrapper.append([prev, next, newConfig]);
    jQuery(".footer").html(wrapper);
  }
  {literal}
  function createHiddenInput(name, defaultValue) {
    const inputExist = jQuery(`input[name="${name}"]`);
    if (inputExist.length) {
      return;
    }
    const input = jQuery("<input>").attr("type", "hidden").attr("name", name);
    if (defaultValue) {
      input.val(defaultValue);
    }

    switchConfiguratorForm.append(input);
  }
  
  function prepareSteps(category) {
    switch (category) {
      case "mechanical":
        steps = ["category", ...stepsMechanical];
        break;
      case "touch":
        steps = ["category", ...stepsTouch];
        break;
      default:
        break;
    }
    steps.forEach((step) => {
      createHiddenInput(step);
    });
  }
  function handleStepInputValue(name, value) {
    const input = jQuery(`input[name="${name}"]`);
    input.val(value);
  }
  function createRadioButton(attributes, handleChange) {
    const radio = jQuery("<input>").attr("type", "radio");
    for (const [key, value] of Object.entries(attributes)) {
      radio.attr(`${key}`, `${value}`);
    }
    radio.on("click", handleChange);
    return radio;
  }
  function createLabel(inputName, labelText, appendElement) {
    return jQuery("<label>")
      .addClass("custom-radio")
      .attr("for", inputName)
      .text(labelText)
      .append(appendElement);
  }
  function handleTotalAmount(multiply) {
    let totalAmount = 0;
    const totalAmountElement = jQuery("#totalAmount");
    const framePrice = jQuery("input[name=frameData]").data("price");
    const boxInserts = jQuery("input[name=boxInsert]");
    boxInserts?.each(function () {
      const { price } = jQuery(this).data();
      if (!isNaN(price)) {
        totalAmount += price;
      }
    });
    totalAmount += framePrice;
    if (multiply) {
      totalAmount = totalAmount * multiply;
	  jQuery('.counter').text(multiply+' x ');
    }
    totalAmountElement.text(`${totalAmount.toFixed(2)}zł`);
  }
  function categoryStepHandle(data) {
    switchConfiguratorForm.hide();
	jQuery(`input[name="color"]`).val('');
	jQuery(`input[name="boxesNumber"]`).val('');
	jQuery(`input[name="boxesTypes"]`).val('');
	jQuery(`input[name="summary"]`).val('');
	jQuery(`input[name="orientation"]`).val('');
    jQuery(".initial-screen").show();
    const div = jQuery("<div>").addClass("d-block").addClass("d-md-flex");
    const title = jQuery("<h2>").text("Wybierz serię produktów");
    const handleChange = (event) => {
		jQuery("#switchConfigurator").removeClass('mechanical');
	  if(event.target.id == 'touch'){
		  jQuery('.steps').html('<span>Krok 1</span><span>Krok 2</span><span>Krok 3</span><span>Krok 4</span><span>Krok 5</span>');
	  }else{
		  jQuery("#switchConfigurator").addClass('mechanical');
		  jQuery('.steps').html('<span>Krok 1</span><span>Krok 2</span><span>Krok 3</span><span>Krok 4</span><span>Krok 5</span><span>Krok 6</span>');
	  }
      const { value } = event.target;
      prepareSteps(value);
      handleStepInputValue("category", value);
      setCurrentStep("color");
      switchConfiguratorForm.show();
      jQuery(".initial-screen").hide();
    };
    if (!jQuery(".initial-screen .custom-radio").length) {
      data.options.forEach((element, index) => {
        const input = createRadioButton(
          {
            name: `${data.name}-input`,
            value: element.value,
            id: element.value,
          },
          handleChange
        );
        const img = jQuery("<img>")
          .attr("src", element.imgUrl)
          .attr("alt", element.title);
        const label = createLabel(element.value, element.title, [input, img]);
        div.append(label);
      });

      jQuery(".initial-screen").append(title, div);
    }
  }
  function colorStepHandle(data) {
    jQuery("#color-picker").on("input", function () {
      const selectedColor = jQuery(this).val();
      leftSideContent.css("background-color", selectedColor);
    });
    const navButtons = jQuery(".nav-button");
    if (navButtons.length < 2) {
      createNavButtons();
    }
    const content = [];
    const handleChange = (event) => {
      const { value, dataset } = event.target;
      handleStepInputValue(data.name, value);

      const url = dataset["img"];
      const img = jQuery("<img>").attr("src", url);
      setLeftSideContent(img);
    };
    data.options.forEach((element, index) => {
      const img = jQuery("<img>").attr("src", element.imgUrl);

      const input = createRadioButton(
        {
          name: `${data.name}-input`,
          value: element.value,
          id: element.value,
          "data-img": element.imgUrl,
        },
        handleChange
      );
      const label = createLabel(element.value, element.title, [input, img]);
      content.push(label);
      if (!index) {
        setLeftSideContent(jQuery("<img>").attr("src", element.imgUrl));
        input.attr("checked", true);
        handleStepInputValue(data.name, element.value);
      }
    });

    setRightSideContent(jQuery("<div>").append(content));
  }
  function materialStepHandle(data) {
    const handleChange = (event) => {
      const { value, dataset } = event.target;
      handleStepInputValue(data.name, value);
      const url = dataset["img"];
      const img = jQuery("<img>").attr("src", url);
      setLeftSideContent(img);
    };
    const content = [];

    data.options.forEach((element, index) => {
      const color = jQuery(`input[name="color"]`).val();
      const { url } = element.images.find((image) => image.color == color);

      const img = jQuery("<img>").attr("src", url);
      const input = createRadioButton(
        {
          name: `${data.name}-input`,
          value: element.value,
          id: element.value,
          "data-img": url,
        },
        handleChange
      );
      const label = createLabel(element.value, element.title, [img, input]);
      content.push(label);

      if (!index) {
        const img = jQuery("<img>").attr("src", url);
        setLeftSideContent(img);
        input.attr("checked", true);
        handleStepInputValue(data.name, element.value);
      }
    });

    setRightSideContent(jQuery("<div>").append(content));
  }
  function createOrientationInputs() {
    const name = "orientation";
    createHiddenInput(name);
    const div = jQuery("<div>");
    const orientationInputValue = jQuery(`input[name=${name}]`).val();

    function handleChange(event) {
      const { value } = event.target;
      handleStepInputValue(name, value);
      const imageElement = leftSideContent.find("img");
      if (value === "vertical") {
        imageElement.addClass("rotate-vertical");
      } else {
        imageElement.removeClass("rotate-vertical");
      }
    }
    const header = jQuery("<h3>").text("Orientacja");
    const inputVertical = createRadioButton(
      { name: `${name}-input`, value: "vertical", id: "vertical" },
      handleChange
    );
    const labelVertical = createLabel(
      "vertical",
      "orientacja pionowa",
      inputVertical
    );
    const inputHorizontal = createRadioButton(
      {
        name: `${name}-input`,
        value: "horizontal",
        id: "horizontal",
      },
      handleChange
    );
    const labelHorizontal = createLabel(
      "horizontal",
      "orientacja pozioma",
      inputHorizontal
    );

    switch (orientationInputValue) {
      case "vertical":
        inputVertical.attr("checked", true);
        break;
      case "horizaontal":
        inputHorizontal.attr("checked", true);
        break;
      default:
        inputHorizontal.attr("checked", true);
        handleStepInputValue(name, "horizontal");

        break;
    }

    return div.append([header, labelVertical, labelHorizontal]);
  }
  function createKitInput(data) {
    const name = "kit";
    createHiddenInput(name);
    const div = jQuery("<div>").addClass('ml-lg-4');
    const kitInputValue = jQuery(`input[name=${name}]`).val();

    function handleChange(event) {
      const { value } = event.target;
      handleStepInputValue(name, value);
    }
    const header = jQuery("<h3>").text("Zestaw");
    const inputWithPlugs = createRadioButton(
      { name: `${name}`, value: "withPlugs", id: "withPlugs" },
      handleChange
    );
    const labelWithPlugs = createLabel(
      "withPlugs",
      "Włączniki z gniazdkami",
      inputWithPlugs
    );
    const inputOnlySwitches = createRadioButton(
      {
        name: `${name}`,
        value: "onlySwitches",
        id: "onlySwitches",
      },
      handleChange
    );
    const labelOnlySwitches = createLabel(
      "onlySwitches",
      "Tylko włączniki",
      inputOnlySwitches
    );

    switch (kitInputValue) {
      case "withPlugs":
        inputWithPlugs.attr("checked", true);
        break;
      case "onlySwitches":
        inputOnlySwitches.attr("checked", true);
        break;
      default:
        inputOnlySwitches.attr("checked", true);
        handleStepInputValue(name, "onlySwitches");

        break;
    }

    return div.append([header, labelWithPlugs, labelOnlySwitches]);
  }

  function handleBoxesNumberStep(data) {
    const rightDiv = jQuery("<div>");
    const labelWrapper = jQuery("<div>").addClass('d-flex').addClass('label-wrapper').addClass('w-100').addClass('justify-content-start');
    const boxesNumberInput = jQuery(`input[name="boxesNumber"]`);
    const categoryInput = jQuery(`input[name="category"]`);
    const initImage = boxesNumberInput.data("imgUrl");
    const initNumberValue = boxesNumberInput.val();
    const isCategoryTouch = categoryInput.val() === "touch";
    createHiddenInput("frameData");
    function setPrevieImage(url, dimensions="") {
      const img = jQuery("<img>").attr("src", url);

      if (jQuery(`input[name="orientation"]`).val() === "vertical") {
        img.addClass("rotate-vertical");
      }
	  
      setLeftSideContent(img, dimensions);
    }
    if (initImage) {
      setPrevieImage(initImage);
    }
    function handleChange(event) {
      const { value } = event.target;
      const { imgUrl, imgUrl2, id, reference, price, width, height, depth, konfigurator_text, konfigurator_schema, readyprod } = jQuery(this).data();
	  const dimensions = jQuery("<div>").addClass("dimensions");
      const widthContent = jQuery("<div>").append("<label>Szerokość:</label><span>"+width+" mm</span>");
      const heightContent = jQuery("<div>").append("<label>Długość:</label><span>"+height+" mm</span>");
	  dimensions.append(widthContent,heightContent);
      setPrevieImage(imgUrl, dimensions);
      boxesNumberInput.val(value).data({'imgUrl':imgUrl, 'imgUrl2':imgUrl2});
      jQuery("input[name=frameData]").val(id).data({'price':price, 'reference':reference, 'readyprod':readyprod});
      handleTotalAmount();
    }

    data.options.map((element, index) => {
      const color = jQuery(`input[name="color"]`).val();
      const material = jQuery(`input[name="material"]`).val();
	  let prodIndex = element.products.findIndex(
        (image) =>
          image.color == color &&
          (isCategoryTouch || (!isCategoryTouch && image.material === material))
      );
	  if(prodIndex > -1){
		  const { url, url2, id, price, reference, width, height, depth, konfigurator_text, konfigurator_schema, readyprod } = element.products.find(
			(image) =>
			  image.color == color &&
			  (isCategoryTouch || (!isCategoryTouch && image.material === material))
		  );
		  const input = createRadioButton(
			{
			  name: `${data.name}-input`,
			  value: element.value,
			  id: `boxes-${element.value}`,
			},
			handleChange
		  );
		  input.data("imgUrl", url);
		  input.data("imgUrl2", url2);
		  input.data("id", id);
		  input.data("price", price);
		  input.data("reference", reference);
		  input.data("readyprod", readyprod);
		  input.data("width", width);
		  input.data("height", height);
		  input.data("depth", depth);
		  if ((initNumberValue && initNumberValue === element.value) || !index) {
			input.trigger("click");

			handleStepInputValue(data.name, element.value);
		  }
		  const label = createLabel(`boxes-${element.value}`, element.value, input);
		  labelWrapper.append(label);
	  }
    });
	rightDiv.append(labelWrapper);
    if (isCategoryTouch) {
	  rightDiv.append(createOrientationInputs());
      rightDiv.append(createKitInput());
    }else{
		rightDiv.append('<p class="mt-lg-5 pt-5"><b>Dowolny montaż: pion/poziom</b></p><p><b>Wszystkie produkty z konfiguratora są kompatybilne ze standardowymi puszkami elektrycznymi</b></p>');
	}

    setRightSideContent(rightDiv);
    jQuery(".amount-wrapper").show();
  }

  function handleBoxesTypesStep(data) {
	jQuery(".nav-button--next").prop("disabled", true);
	const category = jQuery(`input[name="category"]`).val();
    const isCategoryTouch = category === "touch";
    handleTotalAmount();
	let li = '';
	if (isCategoryTouch) {
		li='<li>Określ kolor modułu</li>';
	}
    setRightSideContent(jQuery("<div>").html('<ol class="instruction"><li>Wybierz miejsce w ramce, które chcesz skonfigurować</li><li>Wybierz kategorię modułu</li>'+li+'<li>Wybierz moduł</li></ol>'));
    const orientationInputValue = jQuery(`input[name="orientation"]`).val();
    const boxesNumberElement = jQuery(`input[name="boxesNumber"]`);
    const boxesNumber = boxesNumberElement.val();
	const leftDiv = jQuery("<div>").addClass("boxes-configuration-wrapper").addClass("boxes-configuration-wrapper-"+boxesNumber);
    jQuery(":root").css("--boxes", boxesNumber);
    jQuery(".left-side").css("display", "block");
    const frameImgUrl = boxesNumberElement.data("imgUrl");
    const frameImgUrl2 = boxesNumberElement.data("imgUrl2");
    const presentationDiv = jQuery("<div>")
      .addClass("boxes-configuraiton-preview")
      .addClass("boxes-configuraiton-preview-"+category)
      .attr("data-back",`url(${frameImgUrl2})`)
      .attr("data-front",`url(${frameImgUrl})`)
      .css("background-image", `url(${frameImgUrl})`);
    if (orientationInputValue === "vertical") {
      presentationDiv.addClass("rotate-vertical");
    }
    leftDiv.append(presentationDiv);
    let selectedBoxInsert;
	function createModals(element) {
      const title = jQuery("<h2>").text(element.name).addClass("modal-title").addClass("mb-3");
      const desc = jQuery("<p>").text(element.konfigurator_text).addClass("modal-desc");
      const productUrl = jQuery("<a>")
	  .text('Zobacz produkt')
	  .attr('href', element.productUrl)
	  .attr('target', '_blank')
	  .addClass("see-product");
	  
	  const buttons = jQuery("<div>").addClass("d-flex").addClass("footer-buttons").addClass("mt-4").append(productUrl);
	  if(element.konfigurator_schema && element.konfigurator_schema != ''){
		  const schemeButton = jQuery("<a>")
		  .text('Zobacz schemat')
		  .attr('href', '#')
		  .addClass("see-scheme")
		  .on("click", function (event) {
			  event.preventDefault();
			  
			  $(this).parent().parent().addClass('scheme-mode');
			});
			
			buttons.append(schemeButton);
	  }
      const close = jQuery("<button>")
	  .addClass("close")
	  .attr("data-dismiss", 'modal')
	  .html('<span aria-hidden="true">&times;</span>');
      
	  const content = jQuery("<div>")
	  .addClass("modal-content")
	  .append(close)
	  .append(title)
	  .append(desc)
	  .append(buttons);
		if(element.konfigurator_schema && element.konfigurator_schema != ''){
			const schemeImg = jQuery("<img>")
			  .attr('src', element.konfigurator_schema)
			  .addClass("img-fluid")
			  .addClass("scheme-img")
			  .on("click", function (event) {
				  event.preventDefault();
				  jQuery(this).parent().removeClass('scheme-mode');
				});
			content.append(schemeImg);
		}
		const body = jQuery("<div>")
	  .addClass("modal-body")
	  .append(content);
	  const dialog = jQuery("<div>")
	  .addClass("modal-dialog")
	  .addClass("modal-xl")
	  .append(body);
	  
	  const modal = jQuery("<div>")
        .attr("id", "infoModal-"+element.id)
        .attr("aria-hidden", "true")
        .addClass("modal")
        
        .addClass("fade")
		.append(dialog);

      return modal;
    }
    function createBoxInsertButton(element) {
      const img = jQuery("<img>").attr("src", element.inputUrl).attr("draggable",true);

		img[0].addEventListener("dragstart", (event) => {
			if(!jQuery(event.target).hasClass('checked')){
				jQuery(event.target).click();
			}
			event.dataTransfer.setData("inputUrl", element.inputUrl);
		});
      const title = jQuery("<span>").text(element.name).addClass("title");
      const price = jQuery("<span>")
        .text(`${element.price.toFixed(2)} zł`.replace(".", ","))
        .addClass("price");
	  const moreButton = jQuery("<span>")
        .text(`Więcej informacji`)
        .attr('data-id', element.id)        
		.attr('data-toggle', "modal")
        .attr('data-target', "#infoModal-"+element.id)
        .addClass("open-modal")
        .addClass("p-2")
		
      const button = jQuery("<button>")
        .append(img)
        .append(title)
        .append(price)
        .append(moreButton)
        .addClass("box-insert-button")
        .addClass("color-button")
        .addClass("color-button-"+element.color)
        .on("click", function (event) {
          event.preventDefault();
		  if(jQuery(this).hasClass("checked")){
			  jQuery(".nav-button--next").click();
		  }else{
			  
			  jQuery(".box-insert-button").removeClass("checked");
			  jQuery(this).addClass("checked");
			  selectedBoxInsert = element;
			  jQuery(".nav-button--next").prop("disabled", false);
		  }
		  
        });
      return button;
    }
    function showButtonTypes() {
      const rightDiv = jQuery("<div>").addClass("d-flex column");
	  const kitInputValue = jQuery('input[name="kit"]').val();
	  console.log(kitInputValue);
      const title = jQuery("<span>")
        .text("Wybierz kategorię wkładu")
        .addClass("secondary-title");
      rightDiv.append(title);
      const buttonsWraper = jQuery("<div>").addClass("box-type-buttons-wrapper");
      data.options.forEach((element) => {
		  if(element.type == 'basicPlug' && kitInputValue == 'onlySwitches'){
			  return;
		  }
        buttonsWraper.append(createBoxTypeButton(element));
      });
      rightDiv.append(buttonsWraper);
      setRightSideContent(rightDiv);
    }
    function createBoxTypeButton(element) {
	  const color = jQuery(`input[name="color"]`).val();
	  const categoryInput = jQuery(`input[name="category"]`);
	  const isCategoryTouch = categoryInput.val() === "touch";
      const button = jQuery("<button>")
        .addClass("box-type-button")
        .text(element.name)
        .on("click", function (event) {
          const rightDiv = jQuery("<div>").addClass("d-flex column");
          const title = jQuery("<span>")
            .text(element.name)
            .addClass("secondary-title");
			let colorwrapper = '';
			if(!isCategoryTouch || (isCategoryTouch && element.type == 'basicPlug')){
			  colorwrapper = jQuery("<div>").addClass("justify-content-start").addClass("d-flex").addClass("mb-3").append(
			  '<label class="custom-radio custom-radio-filter '+(color == 19?'checked':'')+'" for="color-filter-19">Złoty<input type="radio" name="color-filter-input" value="19" id="color-filter-19" data-img="/img/customproduct/konfigurator_image1_279.png" '+(color == 19?'checked="checked"':'')+'><img src="/img/customproduct/konfigurator_image1_279.png"></label>'+
			  '<label class="custom-radio custom-radio-filter '+(color == 17?'checked':'')+'" for="color-filter-17">Czarny<input type="radio" name="color-filter-input" value="17" id="color-filter-17" data-img="/img/customproduct/konfigurator_image1_277.png" '+(color == 17?'checked="checked"':'')+'><img src="/img/customproduct/konfigurator_image1_277.png"></label>'+
			  '<label class="custom-radio custom-radio-filter '+(color == 18?'checked':'')+'" for="color-filter-18">Szary<input type="radio" name="color-filter-input" value="18" id="color-filter-18" data-img="/img/customproduct/konfigurator_image1_278.png" '+(color == 18?'checked="checked"':'')+'><img src="/img/customproduct/konfigurator_image1_278.png"></label>'+
			  '<label class="custom-radio custom-radio-filter '+(color == 16?'checked':'')+'" for="color-filter-16">Biały<input type="radio" name="color-filter-input" value="16" id="color-filter-16" data-img="/img/customproduct/konfigurator_image1_276.png" '+(color == 16?'checked="checked"':'')+'><img src="/img/customproduct/konfigurator_image1_276.png"></label>');
			}
		  const wrapper = jQuery("<div>").addClass("box-insert-buttons-wrapper");
          const modalsWrapper = jQuery("<div>").addClass("modals-wrapper");
          event.preventDefault();
          element.data.forEach((element) => {
            wrapper.append(createBoxInsertButton(element));
          });
		  element.data.forEach((element) => {
            modalsWrapper.append(createModals(element));
          });
          const backToTypes = jQuery("<button>")
            .addClass("link")
            .html("&lt; Zmień kategorię")
            .on("click", showButtonTypes);

          rightDiv.append([title, colorwrapper, wrapper, modalsWrapper, backToTypes]);
          setRightSideContent(rightDiv);
		  jQuery('.custom-radio-filter.checked').click();
        });

      return button;
    }
	function allowDrop(ev) {
	  ev.preventDefault();
	}


    function handleAddBox(event) {
      event.preventDefault();
      const target = jQuery(this).data("target");
      const input = jQuery(this).data("productInput");
      showButtonTypes();
      jQuery(".footer").html([
        jQuery("<button>")
          .html("Anuluj")
          .addClass("nav-button nav-button--prev")
          .on("click", (event) => {
            event.preventDefault();
            setRightSideContent(null);
            createNavButtons();
          }),
        jQuery("<button>")
          .html("Zatwierdź")
          .addClass("nav-button nav-button--next")
          .prop("disabled", true)
          .on("click", (event) => {
            event.preventDefault();
            jQuery(`#${target}`)
			  .removeClass("product-filed-207")
			  .removeClass("product-filed-208")
			  .removeClass("product-filed-209")
			  .removeClass("product-filed-210")
              .css("background-image", `url(${selectedBoxInsert.previewUrl})`)
              .attr("data-front", `url(${selectedBoxInsert.previewUrl})`)
              .attr("data-back", `url(${selectedBoxInsert.previewUrl2})`)
              .attr("data-light", `url(${selectedBoxInsert.previewUrl3})`)
              .addClass("product-filed-"+selectedBoxInsert.id)
              .addClass("filed");
            jQuery(`#${target} button span`).text("Edytuj");
            jQuery(`#${input}`).val(selectedBoxInsert.id).data(selectedBoxInsert);
			jQuery(`#${target}`).hover(function(){
			  //jQuery(this).css("background-image", jQuery(this).attr('data-back'))
			  }, function(){
			  //jQuery(this).css("background-image", jQuery(this).attr('data-front'))
			});
            const rightContent = jQuery("<div>").addClass("d-flex column");
            const title = jQuery("<span>")
              .text("Wybrany włącznik")
              .addClass("secondary-title");
            const name = jQuery("<span>")
              .text(selectedBoxInsert.name)
              .addClass("subtitle");
            rightContent.append(title, name);
            setRightSideContent(rightContent);
            handleTotalAmount();
            createNavButtons();
			jQuery(".nav-button--next").prop("disabled", true);
			if(jQuery('input[name="boxesNumber"]').val() == jQuery('.filed').length){
				jQuery(".nav-button--next").prop("disabled", false);
			}
          }),
      ]);
    }

    const exististingBoxInserts = jQuery("input[name=boxInsert]");
    if (exististingBoxInserts.length > parseInt(boxesNumber)) {
      for (
        let index = parseInt(boxesNumber);
        index <= exististingBoxInserts.length;
        index++
      ) {
        jQuery(`#boxInsert-${index}`).remove();
      }
    }

    for (let index = 0; index < parseInt(boxesNumber); index++) {
      const boxData = jQuery(`#boxInsert-${index + 1}`);
      const button = jQuery("<button>").addClass("box-spot-button");
	  button[0].addEventListener("drop", (event) => {
			var data = event.dataTransfer.getData("inputUrl");
			 const target = `boxSpot-${index + 1}`;
			 const input = `boxInsert-${index + 1}`;
				jQuery(`#${target}`)
			  .removeClass("product-filed-207")
			  .removeClass("product-filed-208")
			  .removeClass("product-filed-209")
			  .removeClass("product-filed-210")
              .css("background-image", `url(${selectedBoxInsert.previewUrl})`)
              .attr("data-front", `url(${selectedBoxInsert.previewUrl})`)
              .attr("data-back", `url(${selectedBoxInsert.previewUrl2})`)
              .attr("data-light", `url(${selectedBoxInsert.previewUrl3})`)
              .addClass("filed");
            jQuery(`#${target} button span`).text("Edytuj");
            jQuery(`#${input}`).val(selectedBoxInsert.id).data(selectedBoxInsert);
			jQuery(`#${target}`).hover(function(){
			  //jQuery(this).css("background-image", jQuery(this).attr('data-front'))
			  }, function(){
			  //jQuery(this).css("background-image", jQuery(this).attr('data-front'))
			});
            const rightContent = jQuery("<div>").addClass("d-flex column");
            const title = jQuery("<span>")
              .text("Wybrany włącznik")
              .addClass("secondary-title");
            const name = jQuery("<span>")
              .text(selectedBoxInsert.name)
              .addClass("subtitle");
            rightContent.append(title, name);
            setRightSideContent(rightContent);
            handleTotalAmount();
            createNavButtons();
		});
      const boxSpot = jQuery("<div>")
        .addClass("box-spot")
        .attr("id", `boxSpot-${index + 1}`)
		
        .append(
          button
            .data("target", `boxSpot-${index + 1}`)
            .data("productInput", `boxInsert-${index + 1}`)
            .on("click", handleAddBox)
			.on("dragover", allowDrop)
        );

      presentationDiv.append(boxSpot);
      if (!boxData.length) {
        const input = jQuery("<input>")
          .attr("type", "hidden")
          .attr("name", "boxInsert")
          .attr("id", `boxInsert-${index + 1}`);

        switchConfiguratorForm.append(input);
        button.append(jQuery("<span>").text("Dodaj"));
      } else if (boxData?.data("previewUrl")) {
        button.append(jQuery("<span>").text("Edytuj"));
        boxSpot
		  .removeClass("product-filed-207")
		  .removeClass("product-filed-208")
		  .removeClass("product-filed-209")
		  .removeClass("product-filed-210")
          .css("background-image", `url(${boxData.data("previewUrl")})`)
		  .attr("data-front", `url(${boxData.data("previewUrl")})`)
          .attr("data-back", `url(${boxData.data("previewUrl2")})`)
          .attr("data-light", `url(${boxData.data("previewUrl3")})`)
		  .addClass("product-filed-"+boxData.val())
          .addClass("filed");
		
      } else {
        button.append(jQuery("<span>").text("Dodaj"));
      }
    }
    setLeftSideContent(leftDiv);
	if(jQuery('input[name="boxesNumber"]').val() == jQuery('.filed').length){
		jQuery(".nav-button--next").prop("disabled", false);
	}
    if (
      !jQuery(":root").css("--scale") ||
      jQuery(":root").css("--scale") !== boxesNumber / 5
    ) {
      jQuery(":root").css("--scale", boxesNumber / 5);
    }
	
	//setTimeout(function(){ jQuery('#boxSpot-1 button').click();}, 400);
  }

  function createMultiplyInput() {
    if (!jQuery("input[name=multiply]").length) {
      createHiddenInput("multiply", 1);
    }

    const multiplyInput = jQuery("input[name=multiply]");
    handleTotalAmount(multiplyInput.val());
    multiplyInput.on("input", function () {
      handleTotalAmount(jQuery(this).val());
    });
    const wrapper = jQuery("<div>").addClass(
      "summary-multiply d-flex column border"
    );
    const title = jQuery("<span>").text("Powiel zestaw");
    wrapper.append(title);
    const div = jQuery("<div>").addClass("d-flex");
    const inputValue = jQuery("<span>")
      .text(multiplyInput.val())
      .addClass("multiply-input-value");
    const substarctButton = jQuery("<button>")
      .addClass("multiply-input-button multiply-input-button--minus")
      .html("&minus;")
      .on("click", (event) => {
        event.preventDefault();
        const currentValue = parseInt(multiplyInput.val());
        const newValue = currentValue - 1;
        if (newValue >= 1) {
          inputValue.text(newValue);
          multiplyInput.val(newValue).trigger("input");
		  
        }
      });
    const addButton = jQuery("<button>")
      .addClass("multiply-input-button multiply-input-button--plus")
      .html("&plus;")
      .on("click", (event) => {
        event.preventDefault();
        const currentValue = parseInt(multiplyInput.val());
        const newValue = currentValue + 1;
        inputValue.text(newValue);
        multiplyInput.val(newValue).trigger("input");
      });
    div.append(substarctButton, inputValue, addButton);
    wrapper.append(div);
    return wrapper;
  }
	function arraysEqual(a, b) {
	  if (a === b) return true;
	  if (a == null || b == null) return false;
	  if (a.length !== b.length) return false;

	  // If you don't care about the order of the elements inside
	  // the array, you should sort both arrays here.
	  // Please note that calling sort on an array will modify that array.
	  // you might want to clone your array first.

	  for (var i = 0; i < a.length; ++i) {
		if (a[i] !== b[i]) return false;
	  }
	  return true;
	}
  function handleSummaryStep(data) {
    jQuery(".box-spot-button").hide();
    const rightSideContent = jQuery("<div>").addClass("d-flex column w-100");
    const content = jQuery("<div>").addClass("d-flex flex-wrap justify-content-start w-100");
    const title = jQuery("<span>").text("Skład zestawu").addClass("secondary-title");
	const category = jQuery(`input[name="category"]`).val();
    const isCategoryTouch = category === "touch";
    const boxInserts = jQuery("input[name=boxInsert]");
    const products = jQuery("<div>").addClass("d-flex column summary-products");
    const frameData = jQuery("input[name=frameData]").data();
	const readyArray =  frameData.readyprod.split('|');
	let readyCheckArray =  [];
	if(readyArray.length > 0){
		readyArray.forEach(function(item){
			const elementsArray = item.split(':');
			const itemsArray = elementsArray[0].split(',');
			readyCheckArray[elementsArray[1]] = [];
			itemsArray.forEach(function(prod){
				readyCheckArray[elementsArray[1]].push(prod);
			});
		});
	}
    const frameSummary = jQuery("<div>").addClass("summary-product");
    const { name, price } = frameData;
    frameSummary.append(
      jQuery("<span>").text("Ramka"),
      jQuery("<span>")
        .addClass("bold")
		.text(`${price.toFixed(2)} zł`)
		.prepend(jQuery("<span>").addClass('counter').text('1 x '))

    );
	
    products.append(frameSummary);
	let lastIndex = '';
	let insertsArray = [];
    boxInserts.each(function () {
	  let data = jQuery(this).data();
	  insertsArray.push(data.reference);
	  
      const product = jQuery("<div>").addClass("summary-product");
      const { name, price } = jQuery(this).data();
      if (!name || !price) {
        return;
      }
      product.append(
        jQuery("<span>").text(name),
        jQuery("<span>")
          .text(`${price?.toFixed(2)} zł`)
          .addClass("bold")
		  .prepend(jQuery("<span>").addClass('counter').text('1 x '))
      );
      products.append(product);
    });
	console.log(insertsArray)
	for (const [key, value] of Object.entries(readyCheckArray)) {
		if(arraysEqual(value, insertsArray)){
			lastIndex = key;
		}
	}
	console.log(lastIndex);
    const multiplyInput = createMultiplyInput();
	const notes = jQuery('<div>').addClass('note').addClass('mt-4').addClass('mb-4').html('');
	notes.append('<label>Napisz komentarz lub opinię</label>');
	notes.append('<textarea name="note"></textarea>');
	const atc = jQuery("<button>").addClass('add-to-cart-button').text('Dodaj do koszyka');
    content.append(products, multiplyInput);
	atc.click((e) => {
      e.preventDefault();
	  let comment = 'KONFIGURACJA \n Kolejność produktów:';
	  const multiply = jQuery("input[name=multiply]").val();
	  const note = jQuery("textarea[name=note]").val();
	  let frameData = jQuery("input[name=frameData]").val();
	  const frameDataData = jQuery("input[name=frameData]").data();
	  const orientation = jQuery("input[name=orientation]").val();
	  const boxInserts = jQuery("input[name=boxInsert]");
	  if(lastIndex != 'none' && lastIndex != ''){
		  comment = comment + 'Ramka: '+lastIndex+',\n';
	  }else{
		comment = comment + 'Ramka: '+frameDataData.reference+',\n';
	  }
	   boxInserts.each(function () {	
		  let data = jQuery(this).data();
		  comment = comment + ' '+data.reference+',';
		  
	  });
	  if(orientation == 'horizontal'){
		  comment = comment + '\n Orientacja: pozioma';
	  }else{
		  comment = comment + '\n Orientacja: pionowa';
	  }
	  if(note != ''){
		  comment = comment + '\n Komentarz/opinia:'+note;
	  }
	  let products = [];
	  if(lastIndex != 'none' && lastIndex != ''){
		  let productIndex = productsArray.findIndex(item => item.reference == lastIndex);
		  frameData = productsArray[productIndex].id;
	  }
	  if(frameData){
		  console.log('Adding to cart with token:', prestashop.static_token || prestashop.token);
		  $.ajax({
			type: 'POST',
			url: prestashop.urls.base_url + 'index.php?controller=cart',
			data: {
				ajax: 1,
				action: 'update',
				add: 1,
				qty: parseInt(multiply),  // Custom flag for clarity
				cart_comment: isCategoryTouch ? comment:'',
				id_product: parseInt(frameData), // Send as JSON
				token: prestashop.static_token || prestashop.token // Token dla zalogowanych użytkowników
			},
			dataType: 'json',
			success: function(response) {
				if (response.hasError) {
					console.log("Error adding to cart:", response.errors);
				} else {
					console.log("Products added successfully:", response);
					//alert("Products added to cart!");
					boxInserts.each(function () {		  
					  let product = {
						  qty:parseInt(multiply),
						  id_product: parseInt(jQuery(this).val())
					  }
					  products.push(product);
					  $.ajax({
						type: 'POST',
						url: prestashop.urls.base_url + 'index.php?controller=cart',
						data: {
							ajax: 1,
							action: 'update',
							add: 1,
							qty: product.qty,  // Custom flag for clarity
							id_product: product.id_product, // Send as JSON
							token: prestashop.static_token || prestashop.token // Token dla zalogowanych użytkowników
						},
						dataType: 'json',
						success: function(response) {
							if (response.hasError) {
								console.log("Error adding to cart:", response.errors);
							} else {
								console.log("Products added successfully:", response);
								//alert("Products added to cart!");
							}
						},
						error: function(xhr, status, error) {
							console.log("AJAX Error:", error);
						}
					});
					});
				}
			},
			error: function(xhr, status, error) {
				console.log("AJAX Error:", error);
			}
		});
	  }
	  
		jQuery('.progress-bar').hide();
		jQuery('.footer').hide();
		jQuery('.add-to-cart-button').hide();
		rightSideContent.append('<p class="mt-4"><b>Produkty zostały dodane do koszyka.</b></p>');
		rightSideContent.append('<a href="/koszyk" target="_blank" id="ignorePDF" class="show-cart">Zobacz koszyk</a>');
		rightSideContent.append('<a href="/konfigurator" id="ignorePDF" class="new-config">Nowa konfiguracja</a>');
		
    });
	const qr = jQuery('<div>').attr('id','qrcode');

	const getConfig = jQuery("<a>").addClass('get-config').text('Pobierz konfigurację').attr('href', '#');
		getConfig.click((e) => {
			e.preventDefault();
			const boxInserts = jQuery("input[name=boxInsert]");
			const multiply = jQuery("input[name=multiply]").val();
			const amount = jQuery("#totalAmount").html();
			const frameData = jQuery("input[name=frameData]").val();
			let cartData = {
				multiply:multiply,
				products:[]
			}
			cartData.products.push(parseInt(frameData));
			boxInserts.each(function () {
				cartData.products.push(parseInt(jQuery(this).val()));
			});
			
			$.ajax({
				type: 'POST',
				url: ajaxUrl, // PrestaShop provides ajaxUrl automatically
				data: {
					action: 'generateHash', // The action name to call in the PHP file
					ajax: 1,
					cartData: JSON.stringify(cartData),
				},
				success: function(response) {
					const data = JSON.parse(response);
					var link = 'https://dotykowewlaczniki.pl/konfiguracja?hash='+data.hash;
					const qrcode = new QRCode(document.getElementById('qrcode'), {
					  text: link
					});
					var elementHandler = {
					  '#ignorePDF': function (element, renderer) {
						return true;
					  }
					};
					var source = document.getElementsByClassName("summary-products")[0];
					var preview = document.getElementsByClassName("left-side")[0];
					var qrImage = document.getElementById('qrcode');
					jQuery('.get-config').hide();
					jQuery('.new-config').hide();
					const robotoBase64 = '';
					html2canvas(source,{useCORS: true, allowTaint: true}).then(canvas => {
					html2canvas(preview,{useCORS: true, allowTaint: true}).then(canvas2 => {
					html2canvas(qrImage,{useCORS: true, allowTaint: true}).then(canvas3 => {
						const imgData = canvas.toDataURL('image/png');
						const imgData2 = canvas2.toDataURL('image/png');
						const imgData3 = canvas3.toDataURL('image/png');
						const doc = new jsPDF('p', 'pt', 'a4');
						doc.addFileToVFS('Roboto-Regular.ttf', robotoBase64);
						doc.addFont('Roboto-Regular.ttf', 'Roboto', 'normal');
						doc.setFont('Roboto'); // Set the custom font
						doc.setFontSize(16);
						doc.text('Podsumowanie konfiguracji', 10, 30);
						doc.text(amount,520,30);
						doc.setFontSize(12);
						doc.text('Zeskanuj kod qr, aby dodać konfigurację do koszyka', 305, 58);
						doc.setTextColor(255, 100, 0);
						doc.setFillColor(255, 100, 0); // Green color (like a button)
						doc.rect(10, 45, 110, 18, 'F'); // (x, y, width, height, style)
						var tableHeight = ($(".summary-products").height() * 400) / $(".summary-products").width();
							// Add clickable text inside the "button"
						doc.setTextColor(255, 255, 255); // White text color
						doc.textWithLink('Dodaj do koszyka', 18, 58, { url: link });
						doc.addImage(imgData, 'PNG', 150, 150, 410, tableHeight);
						doc.addImage(imgData2, 'PNG', 10,150, 120 , 120);
						doc.addImage(imgData3, 'PNG', 520,80, $("#qrcode").width()/3.6,$("#qrcode").height()/3.6);
						doc.save('DC Smart Home.pdf');
					})
					})
					});
				}
			});
			

            /*$.ajax({
                url: prestashop.urls.base_url + 'index.php?fc=module&module=konfigurator&controller=generatepdf',
                type: 'POST',
                data: { id_product: idProduct },
                dataType: 'json',
                success: function (response) {
                    if (response.success) {
                        window.location.href = response.pdf_url;
                    } else {
                        alert("Error generating PDF.");
                    }
                },
                error: function () {
                    alert("AJAX error.");
                }
            });*/
			jQuery('.get-config').show();
			jQuery('.new-config').show();
		});
		//rightSideContent.append(getConfig);
    rightSideContent.append(title, content, notes, atc, getConfig,qr);
	
    setRightSideContent(rightSideContent);
  }

  function handleStepData(step) {
    const category = jQuery(`input[name="category"]`).val();
    const data = configuratoeData[category][step];
    setHeader(data?.title);

    switch (step) {
      case "category":
        categoryStepHandle(configuratoeData[step]);
        break;
      case "color":
        colorStepHandle(data);
        break;
      case "material":
        materialStepHandle(data);
        break;
      case "boxesNumber":
        handleBoxesNumberStep(data);
        break;
      case "boxesTypes":
        handleBoxesTypesStep(data);
        break;
      case "summary":
        handleSummaryStep(data);
        break;
      default:
        break;
    }
  }
  function setCurrentStep(nextStep) {
    const boxTypesIndex = steps.indexOf("boxesTypes");
    const boxNumberIndex = steps.indexOf("boxesNumber");
    const nextStepIndex = steps.indexOf(nextStep);

    jQuery(".progress-bar-indicator").animate(
      { width: `${(nextStepIndex / steps.length) * 100}%` },
      500
    );

    if (nextStepIndex < boxTypesIndex) {
      jQuery(".left-side").css("display", "flex");
    }
    if (nextStepIndex < boxNumberIndex) {
      jQuery(".amount-wrapper").hide();
    }
    if (nextStepIndex + 1 === steps.length) {
      jQuery(".nav-button--next").hide();
    } else {
      jQuery(".nav-button--next").show();
    }
    currentStep = nextStep;
    handleStepData(nextStep);
  }
  handleStepData(currentStep);
}
{/literal}
jQuery("#switchConfigurator").ready(initialize);


</script>
<script>
	tippy('.back-button', {
      content: 'Obróć zestaw',
      animation: 'fade',
      placement: 'top',
    });
	tippy('.light-button', {
      content: 'Włącz podświetlenie',
      animation: 'fade',
      placement: 'bottom',
    });

</script>
{/block}
